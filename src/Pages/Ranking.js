    ...
    async function handleSave() {
        try {
            const response = await fetch(process.env.REACT_APP_API + "saveteams", {
                ...
                body: JSON.stringify({ manager: userTeam, ranking: ranking }),
            });
        ...
        <div className="box">
            ...
            >
                <div
                    className="container"
                    onMouseLeave={() => {
                        group.length && chooseteam.current.classList.remove("hover");
                    }}
                >
                ...
                <button
                    className="save-btn"
                    onClick={() => {
                        ranking.some((e) => !e.id) ? alert("შეავსე ბოლომდე") : handleSave();
                    }}
                >
                    save
                </button>
...
export function ChosenGroup({ handleClick, handleDelete, team}) {
    return (
        <div className="ranking-team">
            ...
            <img
                style={!team.logo ? { filter: "invert(100%" } : {}}
                src={
